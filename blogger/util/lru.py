...
def lru_cache_function(max_size=1024, expiration=15*60,
    ...
    """
    def wrapper(func):
        return LRUCachedFunction(func, key, arg_start,
                LRUCacheDict(max_size, expiration))
    return wrapper
...
class LRUCachedFunction(object):
    ...
    """
    def __init__(self, function, key, arg_start, cache=None):
        if cache:
            self.cache = cache
        else:
    ...
    def __call__(self, *args, **kwargs):
        key = (repr( (args[self.arg_start:], kwargs) )
                if self.key is None else self.key)
        try:
